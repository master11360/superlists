...
REPO_URL = 'https://github.com/master11360/superlists.git'
...
def _update_settings(source_folder, site_name):
    ...
    set(setting_path, 'DEBUG = True', 'DEBUG = False')
    set(setting_path, 'ALLOWED_HOSTS =.+$', 'ALLOWED_HOSTS = ["%s"]' % site_name)
...
def _update_virtualenv(source_folder):
    ...
    if not exists(virtual_folder + '/bin/pip'):
        run('virtualenv %s' % virtual_folder)
